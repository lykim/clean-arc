...
public class FindSalesmanByCodeStrategyTest extends BaseUsecaseTest{
	...
	public void findByCode_willReturnData() {
		...
		request.token = token;
		request.salesmanCode = SALESMAN_CODE;
		...
		assertEquals( response.entity.getCode(), SALESMAN_CODE);
		assertEquals( response.entity.getName(), "A01");
		assertEquals( response.entity.getDivision(), "MIYAKO");
	}
