...
public class KpiGatewayImpl implements KpiGateway, TargetKpiGateway {

	private static KpiGatewayImpl INSTANCE;
	...
	private String getWhereQuery(SalesmanRequestModel param) {
		String whereQuery = " WHERE supervisor_code = ? ";
		if(StringUtils.isNotEmpty(param.filterEqual.get("period"))) {
			whereQuery += " AND kp.period=?";
		}
		return whereQuery;
	}
	...
	@Override
	public Page<Salesman> getTargetKpi(FindManyRequestModel param) {
		try(Connection connection = DataSource.getConnection()) {
			String whereQuerycount = getCountWhereQueryKpiPeriod(param);
			String whereQuery = getWhereQuerykpiPeriod(param);
			String orderQuery = getOrderQuery(param);

			PreparedStatement stmt= connection.prepareStatement(getSelectQueryForPagingKpiPeriod(param) + whereQuery + orderQuery);
			setPreparedStatementKpiPeriod(stmt, param);

			ResultSet rs = stmt.executeQuery();  
			List<Salesman> salesmans = mapFullSalesmanFromResultSet(rs);
			
			String countQuery = getCountQueryKpiPeriod();
			PreparedStatement countStmt= connection.prepareStatement(countQuery + whereQuerycount);
			setPreparedStatementKpiPeriod(countStmt, param);
			ResultSet countRs = countStmt.executeQuery(); 
			int total = 0;
			while(countRs.next()){
				total = countRs.getInt("total");
			...
			return new WebPage<>(salesmans, total, param.pageSize);
		}catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("database error",e);
		}
	...
	private void setPreparedStatementKpiPeriod(PreparedStatement stmt, FindManyRequestModel param) throws SQLException {
		int index = 0;
		LocalDate period = null;
		if(StringUtils.isNotEmpty(param.filterEqual.get("period"))) {
			 String stringPeriod = param.filterEqual.get("period");
			 YearMonth yearMonthPeriod = YearMonth.parse(stringPeriod);
			 period = yearMonthPeriod.atDay(1);
			stmt.setDate(++index, period == null ? null : Date.valueOf(period));
		}
		if(StringUtils.isNotEmpty(param.filterEqual.get("nameLike"))) {
			stmt.setString(++index, "%"+param.filterEqual.get("nameLike")+"%");
		}
	}

	private String getCountQueryKpiPeriod() {
		return "SELECT  count(distinct kp.id) as total "
				+ " FROM kpi_period kp "
				+ " LEFT JOIN salesman s ON kp.salesman_id = s.id "
				+ " LEFT JOIN target_kpi tk ON tk.kpi_period_id = kp.id"
				+ " LEFT JOIN unit_kpi uk ON uk.id = tk.unit_kpi_id ";
	}

	private String getSelectQueryForPagingKpiPeriod(FindManyRequestModel param) {
		return "SELECt s.id as s_id, s.active as s_active, s.name as s_name, "
				+ " s.description as s_description, s.*,"
				+ " kp.id as kp_id, kp.*, "
				+ " tk.id as tk_id, tk.*, uk.id as uk_id, "
				+ " uk.name as uk_name, uk.description as uk_description FROM (" + getMainSelectQueryForPagingKpiPeriod(param) + ") kp "
				+ " LEFT JOIN salesman s ON kp.salesman_id = s.id "
				+ " LEFT JOIN target_kpi tk ON tk.kpi_period_id = kp.id"
				+ " LEFT JOIN unit_kpi uk ON uk.id = tk.unit_kpi_id ";
	}
	
	private String getMainSelectQueryForPagingKpiPeriod(FindManyRequestModel param) {
		String selectQuery = "sELECT * FROM kpi_period ";
		return selectQuery + getMainWhereQueryKpiPeriod(param)  + getLimitQueryKpiPeriod(param);
	}
	
	private String getLimitQueryKpiPeriod(FindManyRequestModel param) {
		if(param.pageSize > 0) {
			int offset = ((param.pageNumber * param.pageSize) - param.pageSize);
			return " offset "+offset+" limit "+param.pageSize +" ";
		}
	...
	private String getCountWhereQueryKpiPeriod(FindManyRequestModel param) {
		String whereQuery = " WHERE kp.kpi_status != '" + TargetKpiStatus.DRAFT.getAbbreviation() +"'";
		if(StringUtils.isNotEmpty(param.filterEqual.get("period"))) {
			whereQuery += " AND kp.period=?";
		}
		if(StringUtils.isNotEmpty(param.filterEqual.get("nameLike"))) {
			whereQuery += " AND LOWER(s.name) LIKE LOWER(?) ";
		}
		return whereQuery;
	}
	
	private String getWhereQuerykpiPeriod(FindManyRequestModel param) {
		String whereQuery = "";
		if(StringUtils.isNotEmpty(param.filterEqual.get("nameLike"))) {
			whereQuery += " WHERE LOWER(s.name) LIKE LOWER(?) ";
		}
		return whereQuery;
	}
	
	private String getMainWhereQueryKpiPeriod(FindManyRequestModel param) {
		String whereQuery = " WHERE kpi_status != '" + TargetKpiStatus.DRAFT.getAbbreviation() +"'";
		if(StringUtils.isNotEmpty(param.filterEqual.get("period"))) {
			whereQuery += " AND period=?";
		}
		return whereQuery;
	}
